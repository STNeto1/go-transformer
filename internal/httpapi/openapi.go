...
func buildOpenAPI(addr string) []byte {
	...
	openAPI.AddEndpoints([]*endpoint.EndPoint{
		...
		endpoint.New(
			...
			endpoint.WithSummary("List pipelines"),
			endpoint.WithDescription("Lists saved draft and active pipeline documents."),
			endpoint.WithSuccessfulReturns([]response.Response{response.New(PipelineListResponse{}, "200", "OK")}),
			endpoint.WithErrors([]response.Response{response.New(PipelineErrorResponse{}, "400", "Invalid request"), response.New(PipelineErrorResponse{}, "500", "Server error")}),
			endpoint.WithProduce([]mime.MIME{mime.JSON}),
		),
		endpoint.New(
			endpoint.POST,
			"/pipelines",
			endpoint.WithTags("pipelines"),
			endpoint.WithSummary("Create pipeline draft"),
			endpoint.WithDescription("Creates a saved pipeline draft. Drafts may be incomplete or invalid."),
			endpoint.WithBody(SavePipelineRequest{}),
			endpoint.WithSuccessfulReturns([]response.Response{response.New(PipelineResponse{}, "201", "Created")}),
			endpoint.WithErrors([]response.Response{response.New(PipelineErrorResponse{}, "400", "Invalid request"), response.New(PipelineErrorResponse{}, "409", "Duplicate pipeline"), response.New(PipelineErrorResponse{}, "500", "Server error")}),
		...
		endpoint.New(endpoint.GET, "/pipelines/{id}", endpoint.WithTags("pipelines"), endpoint.WithSummary("Get pipeline"), endpoint.WithDescription("Gets one saved pipeline."), endpoint.WithSuccessfulReturns([]response.Response{response.New(PipelineResponse{}, "200", "OK")}), endpoint.WithErrors([]response.Response{response.New(PipelineErrorResponse{}, "404", "Not found"), response.New(PipelineErrorResponse{}, "500", "Server error")}), endpoint.WithProduce([]mime.MIME{mime.JSON})),
		endpoint.New(endpoint.PATCH, "/pipelines/{id}", endpoint.WithTags("pipelines"), endpoint.WithSummary("Update pipeline draft"), endpoint.WithDescription("Updates a saved pipeline and demotes it to draft."), endpoint.WithBody(SavePipelineRequest{}), endpoint.WithSuccessfulReturns([]response.Response{response.New(PipelineResponse{}, "200", "OK")}), endpoint.WithErrors([]response.Response{response.New(PipelineErrorResponse{}, "400", "Invalid request"), response.New(PipelineErrorResponse{}, "404", "Not found"), response.New(PipelineErrorResponse{}, "409", "Duplicate pipeline"), response.New(PipelineErrorResponse{}, "500", "Server error")}), endpoint.WithConsume([]mime.MIME{mime.JSON}), endpoint.WithProduce([]mime.MIME{mime.JSON})),
		endpoint.New(endpoint.DELETE, "/pipelines/{id}", endpoint.WithTags("pipelines"), endpoint.WithSummary("Delete pipeline"), endpoint.WithDescription("Deletes a saved pipeline."), endpoint.WithSuccessfulReturns([]response.Response{response.New(map[string]bool{}, "200", "Deleted")}), endpoint.WithErrors([]response.Response{response.New(PipelineErrorResponse{}, "404", "Not found"), response.New(PipelineErrorResponse{}, "500", "Server error")}), endpoint.WithProduce([]mime.MIME{mime.JSON})),
		endpoint.New(endpoint.POST, "/pipelines/{id}/activate", endpoint.WithTags("pipelines"), endpoint.WithSummary("Activate pipeline"), endpoint.WithDescription("Validates a saved draft and marks it active if validation succeeds."), endpoint.WithSuccessfulReturns([]response.Response{response.New(PipelineResponse{}, "200", "Activated")}), endpoint.WithErrors([]response.Response{response.New(ValidatePipelineErrorResponse{}, "400", "Invalid pipeline"), response.New(PipelineErrorResponse{}, "404", "Not found"), response.New(PipelineErrorResponse{}, "500", "Server error")}), endpoint.WithProduce([]mime.MIME{mime.JSON})),
